...
INPUT_DIR = "/home/chas/Projects/switchboard-pipeline/all-markup/"


def get_data():
    """Use quality heuristics to create dataset."""

    cha_list = [x for x in os.listdir(INPUT_DIR) if ".cha" in x]
    quality_fnames = []
    for cha_fname in cha_list:
        ...
        with open(INPUT_DIR + cha_fname, 'r', encoding="UTF-8") as cha_file:
        ...
        if quality["@ExactMatch"] > 0.9 and \
           quality["@WordError"] < 0.02:
            ...
            quality_fnames.append(f"{INPUT_DIR}{convo_num}.csv")
